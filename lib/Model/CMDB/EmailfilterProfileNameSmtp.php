...
/**
 * EmailfilterProfileNameSmtp Class Doc Comment
 *
 * @category Class
 * @package  Fortigate\FortiOS
 * @author   OpenAPI Generator team
 ...
 */
class EmailfilterProfileNameSmtp implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = '_emailfilter_profile__name__smtp';
    ...
    const HDRIP_DISABLE = 'disable';
    const HDRIP_ENABLE = 'enable';
     ...
     * @return string[]
     */
    public function getHdripAllowableValues()
    {
        return [
            self::HDRIP_DISABLE,
            self::HDRIP_ENABLE,
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['hdrip'] = $data['hdrip'] ?? null;
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getHdripAllowableValues();
        if (!is_null($this->container['hdrip']) && !in_array($this->container['hdrip'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'hdrip', must be one of '%s'",
                $this->container['hdrip'],
                implode("', '", $allowedValues)
            );
        }
     ...
     * Sets action
     *
     * @param string|null $action Action for spam email.    pass:Allow spam email to pass through.    tag:Tag spam email with configured text in subject or header.    discard:Discard (block) spam email.
     *
     * @return self
     */
    ...
    public function setTagMsg($tag_msg)
    {
        if (!is_null($tag_msg) && (mb_strlen($tag_msg) > 63)) {
            throw new \InvalidArgumentException('invalid length for $tag_msg when calling EmailfilterProfileNameSmtp., must be smaller than or equal to 63.');
        }
     ...
     * @return string|null
     */
    public function getHdrip()
    {
        return $this->container['hdrip'];
    }

    /**
     * Sets hdrip
     *
     * @param string|null $hdrip Enable/disable SMTP email header IP checks for spamfsip, spamrbl and spambwl filters.    disable:Disable SMTP email header IP checks for spamfsip, spamrbl and spambwl filters.    enable:Enable SMTP email header IP checks for spamfsip, spamrbl and spambwl filters.
     *
     * @return self
     */
    public function setHdrip($hdrip)
    {
        $allowedValues = $this->getHdripAllowableValues();
        if (!is_null($hdrip) && !in_array($hdrip, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'hdrip', must be one of '%s'",
                    $hdrip,
     ...
     * Sets local_override
     *
     * @param string|null $local_override Enable/disable local filter to override SMTP remote check result.    disable:Disable local filter to override SMTP remote check result.    enable:Enable local filter to override SMTP remote check result.
     *
     * @return self
     */
